...
uint256 hashGenesisBlock("0xeba12f58c6a16ee3bf449b2748d360f87f95107d0d294fdd39d50c36e9a2162a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // InstaMineNuggetsCLASSA: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "InstaMineNuggetsCLASSA Signed Message:\n";
...
{
    // InstaMineNuggetsCLASSA: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1){
        nSubsidy = nSubsidy * 1500000 * 50  / 100 / 50;
     }

    nSubsidy >>= (nHeight / 7500);


    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 118800;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // InstaMineNuggetsCLASSA: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // InstaMineNuggetsCLASSA: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // InstaMineNuggetsCLASSA: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xeba12f58c6a16ee3bf449b2748d360f87f95107d0d294fdd39d50c36e9a2162a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1424876127;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084989411;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    //// debug print
    printf("InstaMineNuggetsCLASSAMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("InstaMineNuggetsCLASSAMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("InstaMineNuggetsCLASSAMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running InstaMineNuggetsCLASSA with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("InstaMineNuggetsCLASSAMiner terminated\n");
        throw;
    }
}
